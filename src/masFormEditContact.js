    ...
    apexAction.setCallback(this, function (response) {*/
        ...
        //if (component.isValid() && state === "SUCCESS") {
            ...
            //for (var k = 0; k < contactRoles.length; k++) {
                ...
                if (contactsMap[contactRole.Id].ID_Informations__r == undefined) {
                    IdInformations.push({
  ...
  saveContactRole(event, onlySave) {
    ...
    if (isValid) {
        ...
        if (onlySave != true) {
            ...
            var mobileField = component.find("mobileField");

            // Data Validation
            for (let conRole of contactRoles) {
                if ($A.util.isEmpty(conRole.Contact.Phone) && $A.util.isEmpty(conRole.Contact.MobilePhone)) {
                    var toastEvent = $A.get("e.force:showToast");
                    toastEvent.setParams({
                        "type": "error",
                        "title": "Error",
